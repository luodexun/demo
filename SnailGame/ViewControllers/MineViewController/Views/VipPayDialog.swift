...
typealias VipPaySuccessBlock = () -> Void

class VipPayDialog: UIView {
    ...
    var vipPayHandel : VipPaySuccessBlock?
    init() {
    ...
    @objc func submitAction(sender:UIButton) {
        ...
        BusinessEngine.init().vipBuy(psw: payTxtF!.text!, successResponse: { (BaseModel) in
            self.vipPayHandel!()
            self.close()
        }) { (BaseModel) in
