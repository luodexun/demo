...
class TailorPhotoView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let pauseIcon = UIImageView.init(frame: CGRect.init(x: 20*PROSIZE, y: 41*PROSIZE, width: 22*PROSIZE, height: 22*PROSIZE))
        pauseIcon.image = UIImage.init(named: "ic_mine_pause")
        barView.addSubview(pauseIcon)
        
        let pauseTitleLbl = UILabel.init(frame: CGRect.init(x: 52*PROSIZE, y: 45*PROSIZE, width: barView.frame.size.width-72*PROSIZE, height: 17*PROSIZE))
        pauseTitleLbl.text = "我的蜗牛钱包"
