...
class IDCardScaningView: UIView {
    ...
    func addScaningWindow() {
        ...
        let facePathWidth = 150*PROSIZE
        let facePathHeight = facePathWidth * 0.812
        let rect = IDCardScanningWindowLayer?.frame
        facePathRect = CGRect.init(x: rect!.maxX-facePathWidth-35*PROSIZE, y: rect!.maxY-facePathHeight-25*PROSIZE, width: facePathWidth, height: facePathHeight)
        // 人像
        let headIV = UIImageView.init(frame: facePathRect!)
        headIV.image = UIImage.init(named: "icon_idcard_first_head")
