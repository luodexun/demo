...
class AVCaptureViewController: BaseViewController , AVCaptureVideoDataOutputSampleBufferDelegate , AVCaptureMetadataOutputObjectsDelegate {
    
    // 人脸检测框区域
    var faceDetectionFrame : CGRect?

    var avDelegate : AVCaptureDelegate?
    ...
    override func viewDidLoad() {
        ...
        let scaningView = IDCardScaningView.init(frame: self.view.bounds)
        faceDetectionFrame = scaningView.facePathRect
    ...
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        if metadataObjects.count > 0 {
            ...
            let transformedMetadataObject : AVMetadataObject = previewLayer.transformedMetadataObject(for: metadataObject)!
            let faceRegion : CGRect = transformedMetadataObject.bounds
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        if outPutSetting.isEqual(to: NSNumber.init(value: kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange)) || outPutSetting.isEqual(to: NSNumber.init(value: kCVPixelFormatType_420YpCbCr8BiPlanarFullRange)) {
            ...
            if output.isEqual(videoDataOutput) {
                let dict = ArkDwnWallet.idCardRecognit(imageBuffer! , with: session)
                if dict != nil {
                    ...
                    DispatchQueue.main.async {
                        self.avDelegate?.AVCaptureScanBack(info: info!, subImage: subImage)
                        self.navigationController?.popViewController(animated: true)
                    }
                }
                if (videoDataOutput.sampleBufferDelegate != nil) {
                    videoDataOutput.setSampleBufferDelegate(nil, queue: queue)
                }
            }
        } else {
            print("输出格式不支持")
